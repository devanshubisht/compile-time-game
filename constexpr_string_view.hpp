...
#ifndef COMPILE_TIME_GAME_CONSTEXPR_STRING_VIEW_HPP
#define COMPILE_TIME_GAME_CONSTEXPR_STRING_VIEW_HPP
#include <algorithm>
#include <cstddef>
#include <stdexcept>
...
public:
  ...
  constexpr constexpr_string_view substr(std::size_t pos,
                                         std::size_t len = -1) const {
    if (pos > size_) {
      throw std::runtime_error("out of range");
    }

    const std::size_t new_size = (len == static_cast<std::size_t>(-1))
                                     ? (size_ - pos)
                                     : std::min(len, (size_ - pos));
...
#endif // COMPILE_TIME_GAME_CONSTEXPR_STRING_VIEW_HPP
